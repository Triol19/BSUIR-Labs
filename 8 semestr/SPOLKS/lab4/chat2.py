...
class MulticastChat(object):
    ...
    def __init__(self, nick, my_ip):
        ...
        try:
            ...
            #  1 by defaul to prevent them to be forwarded beyond the local network
            ...
            if _DEBUG_:
                ...
                self._s.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_LOOP, 0)  # disable loop back(not recieve self message)
    ...
    def join_room(self, room_number):
        ...
        if ip:
            ...
            if room_number == 0:
                ...
                print "joined room {}".format(room_number)
    ...
    def add_membership(self, ip):
        self._s.setsockopt(socket.IPPROTO_IP,
                            ...
                            socket.inet_aton(ip) +          # IP multicast address of group
                            socket.inet_aton(self._my_ip)   # local IP address of interface
                        )
    ...
    def process(self):
        r, _, _ = select.select([self._s],[],[],0.5)        # use only list for read
        if r:
            ...
            try:
                ...
                if "message" in data:
                    if "nick" in data:
                        print "[{}][{}]{}: {}".format(strftime("%Y-%m-%d %H:%M:%S"), data['room'], data['nick'], data['message'])
    ...
    def send_room(self, message, room_number = 0):
        if room_number not in self._rooms:
            print "Not a member of room {}".format(room_number)
            return
        data = {
            "nick": self._nick,
            "message": message,
    ...
    def parse_command(self,address, command, data):
        if command == "ping":
            ...
            print "\nUser {} joined room {}".format(data['nick'],data['room'])
        elif command == "goodbye":
            print "\nUser {} leaved room {}".format(data['nick'],data['room'])
...
!list <number> - list chat user in room number
!quit - quit a chat
